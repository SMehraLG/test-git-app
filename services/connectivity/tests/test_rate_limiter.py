...
# Integration tests use a small capacity and zero refill so the bucket exhausts
# predictably regardless of how long individual requests take.
_TEST_CAP = 5
_TEST_REFILL = 0.0
...
def fresh_app(mock_bq: MockBigQueryAdapter, mock_llm: MockLLMAdapter):
    ...
    Uses a small capacity with zero refill to eliminate timing sensitivity.
    """
...
async def test_tokens_refill_allows_further_requests(fresh_app) -> None:
    ...
    async with _make_client(fresh_app) as client:
        ...
        for bucket in mw._buckets.values():
            bucket._tokens = float(_TEST_CAP)
